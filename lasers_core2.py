...
class Lasers_GUI(QlasersWindow, Ui_lasersWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.close_gui_push.setStyleSheet('background-color:coral;') #,"background-color:black;"
    ...
    def read_command_millenia_meth(self):
        ...
        bb =  self.millenia.readline().decode()
    ...
    def connect_millenia_meth(self):
        ...
        ressource_millenia = '%s' % current_txt[4:-7]
        print('ressource_millenia =', ressource_millenia)        
        self.millenia = serial.Serial(ressource_millenia, xonxoff=True,baudrate=baud_rate_millenia, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE, timeout=timeout_millenia, write_timeout = 2)
        self.millenia.write_timeout = 2
        
        # # rm = visa.ResourceManager('@py')
        # #self.millenia = rm.open_resource(ressource_millenia) # 'COM16')
        ...
        # # self.millenia.flush(visa.constants.VI_READ_BUF) # only for pyvisa 
    ...
    def query_last_pwr_cmd_millenia_meth(self):
        ...
        bb = query(self.millenia, '?PSET')
        
        val_last_pwr_cmd = float(bb[0:-2])
    ...
    def turn_on_millenia_meth(self):
        ...
        if QtWidgets.QMessageBox.question(None, '', "Are you sure you want to turn Millenia ON?",
                            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
                            QtWidgets.QMessageBox.No) == QtWidgets.QMessageBox.Yes:
            writefunc(self.millenia,'ON') # turn on the Millenia
    ...
    def turn_off_millenia_meth(self):
        ...
        if QtWidgets.QMessageBox.question(None, '', "Are you sure you want to turn Millenia OFF?",
                            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
                            QtWidgets.QMessageBox.No) == QtWidgets.QMessageBox.Yes:
            writefunc(self.millenia,'OFF') # turn off the Millenia
    ...
    def shutter_millenia_meth(self):
        ...
        if self.millenia_shutter_open_radio.isChecked(): # shutter open is checked
            ...
            writefunc(self.millenia,'SHUTTER:1') # open shutter
            
        else: # shutter closed is checked
            
            writefunc(self.millenia,'SHUTTER:0') # close the shutter
    ...
    def set_pwr_prof_millenia_meth(self):
        ...
        writefunc(self.millenia,'P:%.2f' % val_pwr)
    ...
    # def ctl_wrmup_millenia_meth(self):
        ...
        self.millenia.reset_input_buffer() # flush buffer
        self.millenia.reset_output_buffer() # flush orders
            
        bb = query(self.millenia, '?WARMUP%')
        val_wrmup = round(float(bb[0:-2]))
    ...
    # def get_pwr_laser_millenia_meth(self):
        ...
        bb = query(self.millenia, '?P')
        val_pwr = round(float(bb[0:-2]))
    ...
    # def get_shutter_status_millenia_meth(self):
        ...
        stat_shutter = query(self.millenia, '?SHUTTER')
    ...
    # def get_diodes_millenia_push_meth(self):
        ...
        bb = query(self.millenia,'?C1')
        stat_diode1 = float(bb[0:-3]) # ex is 25.36A1 for 1
        
        self.millenia_diode1_LCD.display(stat_diode1)
        
        bb = query(self.millenia,'?C2')
        stat_diode2 = float(bb[0:-3]) # ex is 25.36A1 for 1
    ...
    # def get_diodes_temp_millenia_meth(self):
        ...
        bb = query(self.millenia,'?HS1')
        temp_diode1 = float(bb[0:-3]) # ex is 25.36A1 for 1
        
        self.millenia_diode1_temp_LCD.display(temp_diode1)
        
        bb = query(self.millenia,'?HS2')
        temp_diode2 = float(bb[0:-3]) # ex is 25.36A1 for 1
    ...
    # def get_duration_millenia_meth(self):
        ...
        self.millenia_duration_edt.setText(query(self.millenia,'?ONHRS'))
    ...
    def connect_lktoclock_meth(self):
        ...
        timeout_lktoclock = 0.5 # sec # 500 # ms
        ...
        ressource_lktoclock = '%s' % current_txt[4:-7]
        print('ressource_lktoclock =', ressource_lktoclock)        
        self.lktoclock = serial.Serial(ressource_lktoclock, xonxoff=True,baudrate=baud_rate_lktoclock, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE, timeout=timeout_lktoclock, write_timeout = 2)
        self.lktoclock.write_timeout = 2
        # for pyvisa and not pyvisa-py
        # if current_txt[6] == '(':
        #     current_txt = current_txt[0:6]
        # else:
    ...
    def get_param_status_lktoclock_meth(self):
        ...
        self.lktoclock.reset_input_buffer() # flush buffer
        self.lktoclock.reset_output_buffer() # flush orders
        
        stat_loop_lock = query(self.lktoclock,'LOOP?') # is locked or not
        ...
        if int(stat_loop_lock) == 1: # servo loop on 
            ...
            dly_raw = query(self.lktoclock,'DLY?') # delay set by DLY command
            ...
            LMON_DC = query(self.lktoclock,'LMON:DC?')
            LMON_AC = query(self.lktoclock,'LMON:AC?')
            
            PZT_DC = query(self.lktoclock,'PZT:DC?')
            PZT_AC = query(self.lktoclock,'PZT:AC?')
    ...
    def lktoclock_after_change_dly_spnbx_meth(self):
        ...
        writefunc(self.lktoclock,'DLY %d' % dly_to_write)
    ...
    def lktoclock_lock_meth(self):
        ...
        if self.lktoclock_lock_on_radio.isChecked(): # lock on is checked
            ...
            b = writefunc(self.lktoclock,'LOOP 1') # lock
            ...
            b = writefunc(self.lktoclock,'LOOP 0') # unlock
    ...
    def connect_vanguard_meth(self):
        ...
        timeout_vanguard = 2 # sec # 2000 # ms
        ...
        ressource_vanguard = '%s' % current_txt[4:-7]
        print('ressource_vanguard =', ressource_vanguard)        
        self.vanguard = serial.Serial(ressource_vanguard, xonxoff=True,baudrate=baud_rate_vanguard, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE, timeout=timeout_vanguard, write_timeout = 2)
        self.vanguard.write_timeout = 2
        ...
        self.vanguard.reset_input_buffer() # flush buffer
        self.vanguard.reset_output_buffer() # flush orders
    ...
    def get_param_status_vanguard_meth(self):  
        ...
        stat_shutter = query(self.vanguard,'SHUTter?')
        ...
        # diode 1
        bb = query(self.vanguard,'READ:DIODE1:CURRent?')
        
        stat_diode1 = float(bb[0:-4]) # ex is '0.12A1\n' for 1 if OFF
        
        self.vanguard_diode1_LCD.display(stat_diode1)
        
        # diode 2
        bb = query(self.vanguard,'READ:DIODE2:CURRent?')
        
        stat_diode2 = float(bb[0:-4]) # ex is  for 0
    ...
    # def get_diodes_temp_millenia_meth(self):
        ...
        bb = query(self.vanguard,'READ:DIODE1:TEMPerature?')
        temp_diode1 = float(bb[0:-4]) # 
        
        self.vanguard_diode1_temp_LCD.display(temp_diode1)
        
        bb = query(self.vanguard,'READ:DIODE2:TEMPerature?')
        temp_diode2 = float(bb[0:-4]) # 
    ...
    # heatsinks
        ...
        bb = query(self.vanguard,'READ:DIODE1:HeatSINK?')
        hs_diode1 = float(bb[0:-4]) # 
        
        self.vanguard_HS1_LCD.display(hs_diode1)
        
        bb = query(self.vanguard,'READ:DIODE2:HeatSINK?')
        hs_diode2 = float(bb[0:-4]) # 
        
        self.vanguard_HS2_LCD.display(hs_diode2)
        
        # duration 
        bb = query(self.vanguard,'READ:DIODE1:HOURs?')
        # print(bb[0:-6])
        self.vanguard_duration_edt.setText('Control unit ON for %.2f now' % (float(bb[0:-6])/60/10))
        
        # PCT warmed up ? 
        bb = query(self.vanguard,'READ:PCTWarmedup?')
        val_wrmup = round(float(bb[0:-3]))
    ...
    def turn_on_vanguard_meth(self):
        ...
        if QtWidgets.QMessageBox.question(None, '', "Are you sure you want to turn Vanguard ON?",
                            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
                            QtWidgets.QMessageBox.No) == QtWidgets.QMessageBox.Yes:
            writefunc(self.vanguard,'ON') # turn on the vanguard
    ...
    def turn_off_vanguard_meth(self):
        ...
        if QtWidgets.QMessageBox.question(None, '', "Are you sure you want to turn Vanguard OFF?",
            ...
            writefunc(self.vanguard,'OFF') # turn off the vanguard
    ...
    def shutter_vanguard_meth(self):
        ...
        if self.shutter_open_vg_radio.isChecked(): # shutter open is checked
            ...
            writefunc(self.vanguard,'SHUTter:1') # open shutter
            print('shutter open')
        else: # shutter closed is checked
            
            writefunc(self.vanguard,'SHUTter:0') # close the shutter
